from app.rag_graph import CRISIS_PATTERNS, SENSITIVE_PATTERNS, detect_sensitive, query_refiner_node
...
def test_sensitive_patterns_match() -> None:
    ...
    assert SENSITIVE_PATTERNS.search("accommodation requests from employee")
...
def test_guardrail_semantic_sensitive_phrases_match() -> None:
    ...
    queries = [
        ...
        "i need accommodation for medical issue",
...
def test_detect_sensitive_handles_real_user_sensitive_phrases() -> None:
    ...
    assert detect_sensitive("i need accommodation for medical issue")
